...
from src.label_processing import update_folds_for_training_and_evaluation
import argparse
...
parser.add_argument('-c', '--with_custom_split', action='store_true',
                    help="Perform custom split on training and validation sets (for more details, cf. Williams et al. "
                         "(2018) paper).")
parser.add_argument('-mn', '--model_name', type=str, choices=['ef_williams'],
...
parser.add_argument('-s', '--fixed_num_steps', type=int,
                    help="Number of steps to fix for all sequences. Set to 0 if you want to keep the original number "
                         "of steps.")
parser.add_argument('-lt', '--label_type', type=str, choices=['all', 'coarse', 'present', 'dominant'],
                    help="Type of labels used for model training and evaluation. Possible values: all (for all presence"
...
def main():
    ...
    train_list, valid_list, test_list = \
        update_folds_for_training_and_evaluation(train_list, valid_list, test_list, args.label_type, all_scores,
    ...
    train_model(train_list, valid_list, test_list,
                ...
                args.dropout_rate, args.final_activ, args.learning_rate, args.optimizer,
